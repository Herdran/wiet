...
def kombinejszons(list_, list_of_combinations, x, length, y, dense, z):
    sliding = 0
    for j in range(3):
        for i in range(sliding, length // (3 ** dense) + sliding):
            # list_of_combinations[i + y * 3] += list_[x - y]
            # list_of_combinations[i + y * (length // (3 ** (dense))) * dense] += list_[x - y]
            # list_of_combinations[(j + 1) * i + sliding] += list_[x - y]
            list_of_combinations[i + j * length // (3 ** dense)] += list_[x - y]
            # list_of_combinations[i + y * (length // (3 ** (dense))) * (dense + y - 1)] += list_[x - y]
